...
def exec_testcase_6():
    try:
        ...
        general_logger.info(f"TESTCASE 6 - {description_of_tc} \n")
        timestamp = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
        error_records_folder_name=rf'PO\models\RCA13\error_records\TESTCASE6\{timestamp}'
        res = validate_data(general_logger,error_logger,'PO\models\RCA13\RCA13.json',0,None,True,6,error_records_folder_name)
        if res:
