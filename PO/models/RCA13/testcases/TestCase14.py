...
def exec_testcase_14():
    try:
        ...
        error_records_folder_name=rf'PO\models\RCA13\error_records\TESTCASE14\{timestamp}'
        general_logger.info("Validationg : LF_Short_code should not be null for the shorted invoices")
        nul_res = validate_data(general_logger,error_logger,'PO\models\RCA13\RCA13.json',0,None,False,14,error_records_folder_name)
        general_logger.info("Validationg : LF_Short_code matches Short_Code in Billing Model")
        shortcode_res = validate_data(general_logger,error_logger,'PO\models\RCA13\RCA13.json',1,None,True,14,error_records_folder_name)
        if nul_res & shortcode_res:
            increment_count('pass_tc_count',general_logger,error_logger)

    except Exception as e:
        general_logger.info("Error executing Validation for TestCase - 014, check error log file for more details")
