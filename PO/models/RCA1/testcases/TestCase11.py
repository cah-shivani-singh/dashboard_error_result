...
def exec_testcase_11():
    try:
        ...
        error_records_folder_name=rf'PO\models\RCA1\error_records\TESTCASE11\{timestamp}'
        general_logger.info("Validationg : LF_Short_code should not be null for the shorted invoices")
        nul_res = validate_data(general_logger,error_logger,'PO\models\RCA1\RCA1.json',0,None,False,11,error_records_folder_name)
        general_logger.info("Validationg : LF_Short_code should matches with source")
        shortcode_res = validate_data(general_logger,error_logger,'PO\models\RCA1\RCA1.json',1,None,True,11,error_records_folder_name)
