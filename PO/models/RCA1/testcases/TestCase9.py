...
def exec_testcase_9():
    ...
    try:
        general_logger.info(f"*******************TESTCASE-09 : {description_of_tc} *******************")
        timestamp = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
        error_records_folder_name=rf'PO\models\RCA1\error_records\TESTCASE9\{timestamp}'

        centr_res = validate_data(general_logger,error_logger,'PO\models\RCA1\RCA1.json',0,'CENTRALIZED',False,9,error_records_folder_name)
        hubspk_res = validate_data(general_logger,error_logger,'PO\models\RCA1\RCA1.json',1,'HUBSPOKE',False,9,error_records_folder_name)
        non_centr = validate_data(general_logger,error_logger,'PO\models\RCA1\RCA1.json',2,'NON-CENTRALIZED',True,9,error_records_folder_name)
