...
def exec_testcase_3():
    ...
    try:
        general_logger.info(f"*******************TESTCASE-03 : {description_of_tc} *******************")
        timestamp = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
        error_records_folder_name=rf'PO\models\RCA2\error_records\TESTCASE3\{timestamp}'

        centr_res = validate_data(general_logger,error_logger,'PO\models\RCA2\RCA2.json',0,'CENTRALIZED',False,3,error_records_folder_name)
        hubspk_res = validate_data(general_logger,error_logger,'PO\models\RCA2\RCA2.json',1,'HUBSPOKE',False,3,error_records_folder_name)
        non_centr = validate_data(general_logger,error_logger,'PO\models\RCA2\RCA2.json',2,'NON-CENTRALIZED',True,3,error_records_folder_name)
